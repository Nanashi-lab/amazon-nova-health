...
export default function EditPatientModal({ isOpen, onClose, patient }: EditPatientModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      actions.populateFromPatient(patient);
      setError(null);
      setSubmitting(false);
    }
  }, [isOpen, actions.populateFromPatient, patient]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await appActions.updatePatient(patient.id, {
          ...
          <div className={styles.body}>
            {!state.isDone ? (
                ...
                <div className={styles.row}>
                  ...
                  <div className={styles.fieldSmall}>
                    ...
                    <select
                      className={styles.input}
                      value={state.status}
                      onChange={(e) => actions.setStatus(e.target.value as Patient['status'])}
                    >
              ...
              >
                ...
                <p className={styles.completionText}>Patient updated successfully!</p>
                <p className={styles.completionSub}>{state.name}&apos;s record has been saved.</p>
                <motion.button
